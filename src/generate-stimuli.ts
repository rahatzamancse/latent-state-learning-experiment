 ...
 */
export async function generateCircularStimuli(connectorPath: string, featurePaths: string[], radius: number = 200, canvasWidth: number = 500, canvasHeight: number = 500) {
    ...
    images.slice(1).forEach((image, i) => {
        ...
        const x = center.x + radius * Math.cos(angle) - image.width / 2;
        const y = center.y + radius * Math.sin(angle) - image.height / 2;
